...
export partialPreprocFlags="--bdd --include header.h  --include builtin.h --include $srcPath/include/libc-symbols.h -I $srcPath -I $srcPath/include -I $srcPath/include/sys -I /usr/lib/gcc/x86_64-linux-gnu/4.7/include-fixed --debugInterface --writePI --recordTiming --parserstatistics --errorXML --interface --openFeat=openFeaturesList.txt --adjustLines  --lexdebug"
...
flags() {
	...
	if grep -q "extra/locale/" <<< "$name"; then
		...
		if grep -q "libc/sysdeps/linux/common/ssp" <<< "$name"; then		
			extraFlag="$extraFlag -U__SSP__"
		fi	
		...
		if grep -q "libpthread/nptl" <<< "$name"; then
			...
			dummy="dummy"
			#extraFlag="$extraFlag -I $srcPath/libpthread/linuxthreads.old -I $srcPath/libpthread/linuxthreads.old/sysdeps/pthread -I $srcPath/libpthread/linuxthreads.old/sysdeps/$target -I $srcPath/libpthread/linuxthreads.old/sysdeps/sparc -I $srcPath/libc/sysdeps/linux/$target -I $srcPath/librt"
...
extraFlags="$(flags "$1")"
./jcpp.sh $srcPath/$1.c $extraFlags
